...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            try
            {
                ...
                if (debug != null)
                {
                    ...
                    String debugtext = "\n                                        DEBUG\n" +
                        ...
                        "\nLast Runtime: " + Runtime.LastRunTimeMs.ToString("0.000000");
            ...
            {
                writeLog("Exception caught! Please contact the author of this script with the following text and a detailed description of what you did." +
                    "\nType: " + e.GetType() +
                    "\nSource: " + e.Source +
                    "\nMessage: " + e.Message +
                    "\nStacktrace:\n" + e.StackTrace, true);
        ...
        class ProductionManagement : Extension
        {
            ...
            */
            ...
            Dictionary<String, values> oresD;
            Dictionary<String, values> ingotsD;
            Dictionary<String, values> componentsD;
            Dictionary<String, values> restD;

            /* counters:
             * have quota difference
             * 
             * Ores:
             *  0 stone
             *  1 iron
            ...
            public ProductionManagement(Program pro) : base (pro)
            {
				...
				//differences
				oresD = new Dictionary<string, values>();
                ingotsD = new Dictionary<string, values>();
                componentsD = new Dictionary<string, values>();
                restD = new Dictionary<string, values>();

                counters = new int[30,3];
            ...
            void analyzeInventories()
            {
                ...
                pro.writeLog("ProductionManagement::analyzeInventories instCount: " + (pro.Runtime.CurrentInstructionCount - currInst));
            }
			...
			void calculateDifferences() {
				...
				foreach (String key in oresQ.Keys.ToArray()) {
					...
					if (difference > 0) {
						oresD.Add(key, new values(difference, current / quota));
					}
				}
				...
				foreach (String key in ingotsQ.Keys.ToArray()) {
                    ...
                    float difference = quota - current;
					if (difference > 0) {
						ingotsD.Add(key, new values(difference, current / quota));
					}
				}
				...
				foreach (String key in componentsQ.Keys.ToArray()) {
                    ...
                    float difference = quota - current;
					if (difference > 0) {
						componentsD.Add(key, new values(difference, current / quota));
					}
				}
				...
				foreach (String key in restQ.Keys.ToArray()) {
                    ...
                    float difference = quota - current;
					if (difference > 0) {
						restD.Add(key, new values(difference, current / quota));
					}
				}

                pro.writeLog("ProductionManagement::calculateDifferences instCount: " + (pro.Runtime.CurrentInstructionCount - currInst));
            }
            ...
            public void fetchBlocks()
            {
                ...
                pro.writeLog("C: " + containers.Count + " A: " + assemblers.Count + " R: " + refineries.Count + " I: " + inventoryBlocks.Count);
                pro.writeLog("ProductionManagement::fetchBlocks instCount: " + (pro.Runtime.CurrentInstructionCount - currInst));
            }
            ...
            public void drawOres()
            {
                if (oresScreen.Count != 0)
                {
                    foreach (IMyTextPanel screen in oresScreen)
                    {
                        ...
                        {
                            pro.writeLog("difference in ore screens found!");
            ...
            public void drawIngots()
            {
                if (ingotsScreen.Count != 0)
                {
                    foreach (IMyTextPanel screen in ingotsScreen)
                    {
                        ...
                        {
                            pro.writeLog("difference in ingot screens found!");
                            String[] lines = screen.GetText().Split('\n');
            ...
            public void drawComponents1()
            {
                if (components1Screen.Count != 0)
                {
                    foreach (IMyTextPanel screen in components1Screen)
                    {
                        ...
                        {
                            pro.writeLog("difference in c1 screens found!");
                            String[] lines = screen.GetText().Split('\n');
            ...
            public void drawComponents2()
            {
                if (components2Screen.Count != 0)
                {
                    foreach (IMyTextPanel screen in components2Screen)
                    {
                        ...
                        {
                            pro.writeLog("difference in c2 screens found!");
                            String[] lines = screen.GetText().Split('\n');
                    ...
                    //printing new content
                    contentComp2 = "\n        Components2 quota\n";
                    addLine(ref contentComp2, "Thruster              ", components, componentsQ, "Thruster");
                    addLine(ref contentComp2, "Superconductor        ", components, componentsQ, "Superconductor");
                    addLine(ref contentComp2, "Radio-comm            ", components, componentsQ, "Radio");
                    ...
                    addLine(ref contentComp2, "GravGen               ", components, componentsQ, "GravGen");
            ...
            public void drawToolsQuota()
            {
                if (toolsScreen.Count != 0)
                {
                    foreach (IMyTextPanel screen in toolsScreen)
                    {
                        ...
                        {
                            pro.writeLog("difference in tool screens found!");
                            String[] lines = screen.GetText().Split('\n');
		...
		struct values
		{
            ...
            public values (float difference, float ratio)
            {
